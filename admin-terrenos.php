<?php
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['house-title']) && isset($_POST['house-description']) && isset($_POST['house-price']) && isset($_FILES['house-image'])) {
        ...
        if (!empty($_FILES['house-image']['name'])) {
            ...
            $consultaPublicacion = "INSERT INTO publicaciones (`titulo-publicacion`, `texto-publicacion`, `precio`, `id-apartado`) VALUES ('$houseTitle', '$houseDescription', '$housePrice', '2')";
                ...
                if (substr($imageType, 0, 5) === 'image') {
                    ...
                    if ($imageData !== false) {
                        ...
                        if (mysqli_stmt_execute($stmt)) {
                            ...
                            echo '<script>alert("Terreno agregado correctamente.");</script>';
                            echo '<script>window.location.href = "admin-terrenos.php";</script>';
                            exit();
                        } else {
            ...
            echo '<script>alert("Debe proporcionar un archivo de imagen para el terreno.");</script>';
            echo '<script>window.location.href = "admin-terrenos.php";</script>';
        }
    }
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['delete-house-select'])) {
        ...
        $query = "SELECT `id-publicacion` FROM `publicaciones` WHERE `titulo-publicacion` = '$selectedHouseId' AND `id-apartado` = 2";
        ...
        if ($house) {
            ...
            if ($image) {
                ...
                if (mysqli_query($conexion, $deleteImageQuery)) {
                    ...
                    if (mysqli_query($conexion, $deleteQuery)) {
                        ...
                        header('Location: admin-casas.php');*/
                        echo '<script>alert("Terreno eliminado correctamente.");</script>';
                        echo '<script>window.location.href = "admin-terrenos.php";</script>';
                        //echo 'Casa eliminada correctamente.';
                        exit();
                    } else {
                        // Ocurrió un error al eliminar la casa
                        echo 'Error al eliminar el terreno en la base de datos: ' . mysqli_error($conexion);
                        exit();
                    }
                } else {
            ...
            // No se encontró la casa en la base de datos
            echo 'No se encontró el terreno en la base de datos.';
            exit();
        }
...
<body>
    ...
    <center>
        <div class="content">
            <h2>Administración de terrenos</h2>
            <p>¡Aquí puedes gestionar tus publicaciones de Terrenos!</p>

            <div class="change-form">
                <h3>Agregar Terreno</h3>
                <form action="./admin-terrenos.php" method="post" enctype="multipart/form-data">
                    <label for="house-title">Título del terreno</label>
                    <input type="text" id="house-title" name="house-title" required>
                    
                    <label for="house-description">Descripción del terreno</label>
                    <textarea id="house-description" name="house-description" required></textarea>
                    
                    <label for="house-price">Precio del terreno</label>
                    <input type="number" id="house-price" name="house-price" required>
                    
                    <label for="house-image">Imagen del terreno</label>
            ...
            <div class="change-form">
                <h3>Eliminar Terreno</h3>
                <form action="admin-terrenos.php" method="post">
                    <label for="delete-house-select">Seleccionar Terreno</label>
                    <select id="delete-house-select" name="delete-house-select" class="form-select select-large">
                        <?php
                            ...
                            // Realizar consulta a la base de datos para obtener las casas con id de apartado igual a 1
                            $query = "SELECT * FROM `publicaciones` WHERE `id-apartado` = 2";
                            $result = mysqli_query($conexion, $query);
                    ...
                    <button type="submit" class="form-button">Eliminar Terreno</button>
                </form>
            </div>
