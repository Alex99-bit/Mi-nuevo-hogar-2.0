<?php
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['house-title']) && isset($_POST['house-description']) && isset($_FILES['house-image'])) {
        ...
        if (!empty($_FILES['house-image']['name'])) {
            ...
            $consultaPublicacion = "INSERT INTO publicaciones (`titulo-publicacion`, `texto-publicacion`, `precio`, `id-apartado`) VALUES ('$houseTitle', '$houseDescription', '$housePrice', '4')";
                ...
                if (substr($imageType, 0, 5) === 'image') {
                    ...
                    if ($imageData !== false) {
                        ...
                        if (mysqli_stmt_execute($stmt)) {
                            ...
                            echo '<script>alert("Publicación agregada correctamente.");</script>';
                            echo '<script>window.location.href = "admin-hipotecas.php";</script>';
                            exit();
                        } else {
            ...
            echo '<script>alert("Debe proporcionar un archivo de imagen para la publicación.");</script>';
            echo '<script>window.location.href = "admin-hipotecas.php";</script>';
        }
    }

    // Verificar si se ha enviado el formulario de edición
    if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['house-id']) && isset($_POST['house-title']) && isset($_POST['house-description'])) {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['delete-house-select'])) {
        ...
        $query = "SELECT `id-publicacion` FROM `publicaciones` WHERE `titulo-publicacion` = '$selectedHouseId' AND `id-apartado` = 4";
            ...
            if ($image) {
                ...
                if (mysqli_query($conexion, $deleteImageQuery)) {
                    ...
                    if (mysqli_query($conexion, $deleteQuery)) {
                        ...
                        echo 'Error al eliminar la publicación en la base de datos: ' . mysqli_error($conexion);
                        exit();
                    }
                } else {
                    // Ocurrió un error al eliminar la imagen de la publicación
                    echo 'Error al eliminar la imagen de la publicación en la base de datos: ' . mysqli_error($conexion);
                    exit();
                }
            } else {
                // No se encontró la imagen asociada a la publicación
                echo 'No se encontró la imagen asociada a la publicación en la base de datos.';
                exit();
            }
        } else {
            // No se encontró la publicación en la base de datos
...
<body>
    ...
    <center>
        <div class="content">
            ...
            <p>¡Aquí puedes gestionar tus publicaciones de Créditos Hipotecarios!</p>

            <div class="change-form">
                <h3>Agregar Publicación</h3>
                <form action="./admin-hipotecas.php" method="post" enctype="multipart/form-data">
                    <label for="house-title">Título de la publicación</label>
                    <input type="text" id="house-title" name="house-title" required>
                    
                    <label for="house-description">Descripción de la publicación</label>
                    ...
                    <label for="house-image">Imagen de la publicación</label>
            ...
            <div class="change-form">
                ...
                <form action="admin-hipotecas.php" method="post">
                    <label for="delete-house-select">Seleccionar Publicación</label>
                    <select id="delete-house-select" name="delete-house-select" class="form-select select-large">
                        <?php
                            ...
                            // Realizar consulta a la base de datos para obtener las casas con id de apartado igual a 1
                            $query = "SELECT * FROM `publicaciones` WHERE `id-apartado` = 4";
                            $result = mysqli_query($conexion, $query);
                    ...
                    <button type="submit" class="form-button">Eliminar Publicación</button>
                </form>
            </div>
